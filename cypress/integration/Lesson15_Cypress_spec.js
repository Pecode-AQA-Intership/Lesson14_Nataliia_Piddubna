...
describe('Create a new user in box "Web Tables"', () => {
  ...
  it('Create a new user and verify that user was added', () => { 
    ...
    cy.get(SELECTOR_DATA.buttonEdit).click({timeout: 5000});//click the button 'Edit'.'Edit user and check if editing is completed'
    ...
    cy.get(SELECTOR_DATA.department).clear().type(FAKER_DATA.editDepartment);
    cy.get(SELECTOR_DATA.buttonSubmit).click({timeout: 3000});//click the button 'Submit'
    cy.get(SELECTOR_DATA.editTable).parent().should('contain',FAKER_DATA.editFirstName).and('contain',FAKER_DATA.editLastName)
    .and('contain',FAKER_DATA.editEmail).and('contain',FAKER_DATA.editAge).and('contain',FAKER_DATA.editSalary)
    .and('contain',FAKER_DATA.editDepartment)
  });
});
